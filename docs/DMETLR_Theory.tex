...
alone should reproduce the non-interacting limit. In this case, the matrix consists of only one function, and thus, the linear response function can be
written as
\begin{equation}
...
V &= \alpha^{+} \alpha  .
\end{align}
The hamiltonian matrix element of Eq.~\ref{1eCVDiag} is taken to be in the canonical basis (removing the `zeroth' energy contribution as discussed above),
\begin{equation}
E = \frac{1}{N} \sum_{ia} G_{ai} G_{ia} (\epsilon_a - \epsilon_i)
\end{equation}
We find that
\begin{equation}
\langle 0 | p^{+} q | 1 \rangle = \frac{1}{\sqrt{N}} G_{ia}
\end{equation}
