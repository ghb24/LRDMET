...
a quantum impurity coupled to a set of frequency dependent bath states.
...
\end{equation}
with the one- and two-particle Greens functions defined with $V$ and $X$ being single annihilation/creation or pairs of such operators 
...
%the central variable is the local, one-particle Greens function, which is self-consistently embedded within a non-interacting or 
%mean-field Greens function over the system. 
The quantum impurity problem, which is then solved via an `impurity solver', such as exact
continuous-time quantum Monte Carlo (CT-QMC)\cite{Millis2006} or exact diagonalization (ED)\cite{ZgidPRB}. 
...
of computer-hours to stabilize, even for a modest number of impurity sites\cite{}.
...
took more than a few seconds on a single computing core), while eliminating bath discretization error of finite DMFT calculations.
...
phase transitions in the thermodynamic limit, and models the physics of many transition metal oxides\cite{Limelette2003}, including qualitative features 
...
Ansatz\cite{Lieb68,Ovchinni1970}, as well as large-scale 1D and 2D DMFT calculations at half-filling and under doping\cite{Go2009,Kotliar2008,xxx}. 
...
\item $H'|\Psi^{(0)} \rangle = E_0 |\Psi^{(0)} \rangle$ is solved for the quantum impurity and bath wavefunction, $|\Psi^{(0)} \rangle$.
...
\end{equation}
which in this work is solved via an exact, iterative procedure\cite{}.
...
%For one-particle Greens functions,
...
of the hybridization, as described in Ref.~\onlinecite{Go2009}. Both DMET and DMFT calculation are performed in the paramagnetic phase. 
...
features present in the CDMFT study of Ref.~\onlinecite{xxx} are observed (see Fig. 1(c) and (d)). Aside: Also look at A. Liebsch DMFT in doped Hubbard.}
\label{2D_Doped}
\end{figure}
...
at finite $U$, representative of a Mott transition\cite{Georges1996,Senechal2000,Kotliar2008,Millis2012,Millis2013,ZgidPRB?}.
...
elsewhere\cite{}. Cluster DMFT calculations with the same plaquette size observe a MIT at a slightly lower *** Give range - look at PRB *** $U\approx5.5t$, 
with a coexistence region at low temperatures, which we also observe\cite{Kotliar2008}. However, analytically continued
...
until the system becomes a Fermi liquid phase. The spectra are qualitatively similar to those seen in recent cluster DMFT studies\cite{PRL 102 056404 (2009)}.
