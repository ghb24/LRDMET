...
%Spectral functions are defined as $A(\omega)=-\frac{1}{\pi}\Im[G(\omega)]$.
%where the spectral broadening is given by the small imaginary component of the energy, $\eta$, which regularizes the correlation function. 
The single particle
...
%with frequency independent energies and couplings\cite{Liebsch2012}. 
...
%encapsulates many of the challenges in 
...
%, including the full impurity space.
...
%Introducing a frequency dependence for $u$ will not change the computational 
%scaling of the spectral DMET.  
...
%In this Letter, we consider the one- and two-particle local Green functions,
...
%the spectra are obtained on the real axis at zero-temperature, and so can be directly compared against. 
...
%Another point to note is that the sum rules on the spectral functions are
...
%, as have been observed elsewhere\cite{Kotliar2008}. 
...
%The spectra are qualitatively similar to those seen in recent cluster DMFT studies\cite{Masatoshi2009}.
...
%since holons and doublons do not bind in the 1D Hubbard Hamiltonian, 
...
of Fig.~\ref{1D_GAP},
%as before\cite{Essler91}. Therefore, excitation gaps
%obtained from the two-particle Green functions in the 1D model are included in Fig.~\ref{1D_GAP}, 
...
%Extensions are now underway for the
