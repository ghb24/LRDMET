...
def mb_ph_ops(hlat,perturb,omega,nimp,nocc,pemb,pcore,pvirt):
    ...
    if project:
        ...
        for imp in sites_imp:
            ...
            # normalize
            coeff_c/=scipy.linalg.norm(coeff_c)
            cops.append(models.ContractedC(sites_ext,coeff_c))

            coeff_d=t1amp_all_sp[imp,sites_ext]
            # normalize
            coeff_d/=scipy.linalg.norm(coeff_d)
            dops.append(models.ContractedD(sites_ext,coeff_d))
            
            if imp & 1:
                coeff_cb=t1amp_all_sp[sites_ext,imp]
                # normalize
                coeff_cb/=scipy.linalg.norm(coeff_cb)
                copsb.append(models.ContractedC(sites_ext,coeff_cb))

                coeff_db=t1amp_all_sp[imp,sites_ext]
                # normalize
                coeff_db/=scipy.linalg.norm(coeff_db)
                dopsb.append(models.ContractedD(sites_ext,coeff_db))
            else:
                coeff_ca=t1amp_all_sp[sites_ext,imp]
                # normalize
                coeff_ca/=scipy.linalg.norm(coeff_ca)
                copsa.append(models.ContractedC(sites_ext,coeff_ca))

                coeff_da=t1amp_all_sp[imp,sites_ext]
                # normalize
                coeff_da/=scipy.linalg.norm(coeff_da)
                dopsa.append(models.ContractedD(sites_ext,coeff_da))


    cd_coeffs={}
    ext_t1amp_all_sp=t1amp_all_sp[sites_ext,:][:,sites_ext]
    norm_ext=scipy.linalg.norm(ext_t1amp_all_sp)
    for i in sites_ext:
        for j in sites_ext:
            if abs(t1amp_all_sp[i,j]) > 1.e-8:
                #cd_coeffs[i,j]=t1amp_all_sp[i,j]
                # use normalized cd coeffs
                cd_coeffs[i,j]=t1amp_all_sp[i,j]/norm_ext
...
def mb_configs(nsites,nimp,ne_imp_sp,ncore_sp,sz):
    ...
    npa_configs=fermions.conserving_configs(nimp_sp*2,ndim,ne_imp_sp+1,sz+1)
    npb_configs=fermions.conserving_configs(nimp_sp*2,ndim,ne_imp_sp+1,sz-1)

    nm_configs=fermions.conserving_configs(nimp_sp*2,ndim,ne_imp_sp-1,sz+1)
    nm_configs+=fermions.conserving_configs(nimp_sp*2,ndim,ne_imp_sp-1,sz-1)

    nma_configs=fermions.conserving_configs(nimp_sp*2,ndim,ne_imp_sp-1,sz+1)
    nmb_configs=fermions.conserving_configs(nimp_sp*2,ndim,ne_imp_sp-1,sz-1)
...
def mb_ph_ops_configs(ops_dict,configs_dict):
    ...
    for op in ops_dict["ca"]:
        ops_configs+=[(op,configs_dict["n-1b"])]
    for op in ops_dict["cb"]:
        ops_configs+=[(op,configs_dict["n-1a"])]

    for op in ops_dict["da"]:
        ops_configs+=[(op,configs_dict["n+1a"])]
    for op in ops_dict["db"]:
        ops_configs+=[(op,configs_dict["n+1b"])]
