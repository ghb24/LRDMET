...
integer , parameter :: iGlobalTimerLevel = 30  !Default level for timing
integer , parameter :: nPrintTimer = 7 !Number of timers to write out at the end
...
!$        t(2)=OMP_get_wtime() 
          ...
          write(6,"(A)") 'Global timing never initialised via call to init_timing.'
      end if
      ...
      if (timer_level.gt.iGlobalTimerLevel) then
          ...
          if (.not.associated(proc_timer%store)) then
              ...
              if (itimer.gt.ntimer) then
                  write(6,"(A)") 'ntimer parameter too small for the number of objects to be timed.'
                  proc_timer%time=.false.
                  timer_error=.true.
                  return
              end if
...
!$            t(2)=OMP_get_wtime() 
   ...
   subroutine halt_timer(proc_timer)
      ...
      if (.not.proc_timer%time) then
          ...
          write(6,"(A)") 'proc_timer not intialised: '//proc_timer%timer_name
          timer_error=.true.
      else
          call cpu_time(t(1))
          t(2)=0
