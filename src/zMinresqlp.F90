...
!   g95 -c -g -O0 -pedantic -Wall -Wextra -fbounds-check -ftrace=full zminresqlpModule.f90
!   See also: Makefile 
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
contains
  ...
  subroutine MINRESQLP( n, Aprod, b, shift, Msolve, disable, nout,        &
    ...
    !         min || Ax - b ||_2,
    !
    !     where A is an n by n Hermitian matrix and b is a given vector.
    ...
    !        of iterations as MINRESQLP but less work per iteration.
    !
    !     2. If A is indefinite but Ax = b is known to have a solution
    !        (e.g. if A is nonsingular), SYMMLQ might be preferred,
    !        since it requires the same number of iterations as MINRESQLP
    ...
    !                        
    !                        NOTE.  The program calling MINRESQLP must declare
    !                        Aprod to be external.
    !
    ...
    !                        nout is not open properly, results will be written
    ...
    !                        If disable = .true., then a null vector obtained, 
    !                        given rtol. 
    !
    !
    ...
    intrinsic :: abs, sqrt, aimag, present, floor, log10
...
!    real(dp) , external :: znrm2
    ...
    if (present(maxxnorm)) then
       maxxnorm_ = min(maxxnorm, NORMMAX)
    else
       maxxnorm_ = NORMMAX
    end if
    ...
    if (checkA_  .and.  precon_) then
       call Msolve( n, y, r2 )
       s      = zdotc(n, r2, 1, y, 1)
       t      = conjg(zdotc(n, y, 1, r2, 1))
       z      = abs( s - t )
       epsa   = (abs(s) + eps) * eps**0.33333_dp
       !write(*,*) 's = ', s, ', t = ', t, ' z = ', z, ', epsa = ', epsa
       if (z > epsa) then
          istop_ = 10
       end if
    end if
    ...
    do while (istop_ <= flag0) 
       ...
       gamal2 = gamal
       gamal  = gama
       call symortho(gbar, cmplx(betan,zero,dp), cs, sn, gama)
       gama_tmp = gama;
       taul2  = taul
       taul   = tau
       tau    = cs * phi
       ...
       if (abs(gama) > eps  .and.  xnorm_tmp < maxxnorm_) then
          if (debug) then
             write(*,zdebugStr2) 'tau_',   itn_, ' = ', real(tau),   ' + I * ', aimag(tau) ,   ', ', & 
                                 'eta_',   itn_, ' = ', real(eta),   ' + I * ', aimag(eta) ,   ', ', &
          ...
          vec2(1)   = xnorm_tmp
          vec2(2)   = u
          if (znrm2(2, vec2, 1) > maxxnorm_) then
             u      = zero
             istop_ = 12    
          end if
       else     
       ...
       if (debug) then
          ...
          write(*,zdebugStr2) 'gmin_',   itn_,   ' = ', real(gmin), ' + I * ', aimag(gmin)
          write(*,ddebugStr2) 'pnorm_',  itn_,   ' = ', pnorm,   ', ', &
                              'rnorm_',  itn_,   ' = ', rnorm_,   ', ', &
