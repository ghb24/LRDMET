...
SUBROUTINE uobyqa_2(n, x, rhobeg, rhoend, iprint, maxfun, f, calfun_2, n_dum, n2_dum,vecs_dum)
...
INTEGER, INTENT(IN)        :: maxfun
REAL(dp), INTENT(OUT)      :: f
INTEGER, INTENT(IN)        :: n_dum,n2_dum
COMPLEX(dp), INTENT(IN)    :: vecs_dum(n_dum,n2_dum)

INTERFACE
  SUBROUTINE calfun_2(x, f, n,n_dum, n2_dum, vecs_dum)
    use const, only: dp
    use Globals, only: nImp
    IMPLICIT NONE
...
npt = (n*n + 3*n + 2) / 2
CALL uobyqb_2(n, x, rhobeg, rhoend, iprint, maxfun, f, npt, calfun_2, n_dum, n2_dum,vecs_dum)
RETURN
END SUBROUTINE uobyqa_2

!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% uobyqb.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SUBROUTINE uobyqb_2(n, x, rhobeg, rhoend, iprint, maxfun, f, npt, calfun_2, n_dum, n2_dum,vecs_dum)
...
END IF
nf = nf + 1
CALL calfun_2(x, f, n,n_dum, n2_dum, vecs_dum)
IF (iprint == 3) THEN
  WRITE(*, 5100) nf, f, x(1:n)
END IF
IF (nf <= npt) GO TO 70
...
SUBROUTINE minim_2(p, step, nop, func, maxfn, iprint, stopcr, nloop, iquad,  &
                 simp, var, functn, ifault, n_dum, n2_dum, vecs_dum)
...
!Additional information passed in, just to get to the function evaluator
integer, intent(in) :: n_dum
integer, intent(in) :: n2_dum
complex(dp), intent(in) :: vecs_dum(n_dum,n2_dum)

INTERFACE
  SUBROUTINE functn(p, func, nop, n_dum, n2_dum, vecs_dum)
    use const, only: dp
    IMPLICIT NONE
    integer, intent(in) :: n_dum,n2_dum,nop
    REAL (dp), INTENT(IN)  :: p(nop)
    REAL (dp), INTENT(OUT) :: func
    complex(dp), intent(in) :: vecs_dum(n_dum,n2_dum)
  END SUBROUTINE functn
...
IF (nap <= 0) THEN
  CALL functn(p,func, nop, n_dum, n2_dum, vecs_dum)
  RETURN
END IF
...
DO i = 1, np1
  p = g(i,:)
  CALL functn(p,h(i), nop, n_dum,n2_dum,vecs_dum)
  neval = neval + 1
  IF (iprint > 0) THEN
    WRITE (lout,5100) neval, h(i), p
    call flush(lout)
...
!     HSTAR = FUNCTION VALUE AT PSTAR.
  ...
  pstar = a * (pbar - g(imax,:)) + pbar
  CALL functn(pstar,hstar,nop,n_dum,n2_dum,vecs_dum)
  neval = neval + 1
  IF (iprint > 0) THEN
    IF (MOD(neval,iprint) == 0) then
        WRITE (lout,5100) neval, hstar, pstar
...
!     HSTST = FUNCTION VALUE AT PSTST.
  ...
  IF (hstar < hmin) THEN
    pstst = c * (pstar - pbar) + pbar
    CALL functn(pstst,hstst, nop,n_dum,n2_dum,vecs_dum)
    neval = neval + 1
    IF (iprint > 0) THEN
      IF (MOD(neval,iprint) == 0) then
          WRITE (lout,5100) neval, hstst, pstst
  ...
  DO
    ...
    IF (test < simp) THEN
      DO j = 1, nop
        ...
        pstst(j) = g(i,j)
      END DO
      CALL functn(pstst,h(i), nop,n_dum,n2_dum,vecs_dum)
      nmore = nmore + 1
      neval = neval + 1
      IF (h(i) >= hmin) CYCLE
      hmin = h(i)
...
DO i = 1, nap
  i1 = i + 1
  pstar = (g(1,:) + g(i1,:)) * half
  CALL functn(pstar,aval(i), nop,n_dum, n2_dum,vecs_dum)
  nmore = nmore + 1
  neval = neval + 1
END DO
...
END SUBROUTINE minim_2

SUBROUTINE minim_3(p, step, nop, func, maxfn, iprint, stopcr, nloop, iquad,  &
                 simp, var, functn, ifault, n_dum, vecs_dum)
...
!Additional information passed in, just to get to the function evaluator
integer, intent(in) :: n_dum
complex(dp), intent(in) :: vecs_dum(n_dum,2)

INTERFACE
  SUBROUTINE functn(p, func, nop, n_dum, vecs_dum)
    use const, only: dp
    IMPLICIT NONE
    integer, intent(in) :: n_dum,nop
    REAL (dp), INTENT(IN)  :: p(nop)
...
IF (nap <= 0) THEN
  CALL functn(p,func, nop, n_dum, vecs_dum)
  RETURN
END IF
...
DO i = 1, np1
  p = g(i,:)
  CALL functn(p,h(i), nop, n_dum,vecs_dum)
  neval = neval + 1
  IF (iprint > 0) THEN
    WRITE (lout,5100) neval, h(i), p
    call flush(lout)
...
!     HSTAR = FUNCTION VALUE AT PSTAR.
  ...
  pstar = a * (pbar - g(imax,:)) + pbar
  CALL functn(pstar,hstar,nop,n_dum,vecs_dum)
  neval = neval + 1
  IF (iprint > 0) THEN
    IF (MOD(neval,iprint) == 0) then
        WRITE (lout,5100) neval, hstar, pstar
...
!     HSTST = FUNCTION VALUE AT PSTST.
  ...
  IF (hstar < hmin) THEN
    pstst = c * (pstar - pbar) + pbar
    CALL functn(pstst,hstst, nop,n_dum,vecs_dum)
    neval = neval + 1
    IF (iprint > 0) THEN
      IF (MOD(neval,iprint) == 0) then
          WRITE (lout,5100) neval, hstst, pstst
  ...
  DO
    ...
    IF (test < simp) THEN
      DO j = 1, nop
        ...
        pstst(j) = g(i,j)
      END DO
      CALL functn(pstst,h(i), nop,n_dum,vecs_dum)
      nmore = nmore + 1
      neval = neval + 1
      IF (h(i) >= hmin) CYCLE
      hmin = h(i)
...
DO i = 1, nap
  i1 = i + 1
  pstar = (g(1,:) + g(i1,:)) * half
  CALL functn(pstar,aval(i), nop,n_dum, vecs_dum)
  nmore = nmore + 1
  neval = neval + 1
END DO
