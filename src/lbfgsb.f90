...
!============================================================================= 
      ...
      real(dp) :: wa(2*m*n + 5*n + 11*m*m + 8*m), dsave(29)
...
!-jlm-jn
      ...
      real(dp) ::                 wt(m, m), wn(2*m, 2*m), snd(2*m, 2*m), dsave(29)
...
!     ************
      ...
      real(dp) :: theta,fold,ddot,dr,rr,tol,                        &
...
!     Save local variables.
 ...
 1006 format (/,' Nonpositive definiteness in Cholesky factorization in formk;',/,  &
      '   refresh the lbfgs memory and restart the iteration.')
 1007 format (/,' Nonpositive definiteness in Cholesky factorization in formt;',/,  &
      '   refresh the lbfgs memory and restart the iteration.')
...
!     **********
      ...
      real(dp), parameter :: ftol=1.0d-3
      real(dp), parameter :: gtol=0.9d0
      real(dp), parameter :: xtol=0.1d0
...
!======================= The end of prn1lb =============================
      ...
      integer ::          n, iprint, itfile, iter, nfgv, nact, nseg,    &
                       iword, iback
      real(dp) :: f, sbgnrm, stp, xstep, x(n), g(n)
...
!======================= The end of prn2lb =============================
      ...
      real(dp) :: f, sbgnrm, time, stp, xstep, cachyt, sbtime,  &
                       lnscht, x(n)
...
!     **********
      ...
      real(dp), parameter :: p66=0.66d0
      real(dp), parameter :: xtrapl=1.1d0
      real(dp), parameter :: xtrapu=4.0d0
...
!====================== The end of dcsrch ==============================
      ...
      subroutine dcstep(stx,fx,dx,sty,fy,dy,stp,fp,dpx,brackt,   &
                        stpmin,stpmax)
      logical :: brackt
      real(dp) :: stx,fx,dx,sty,fy,dy,stp,fp,dpx,stpmin,stpmax
!     **********
!
!     Subroutine dcstep
!
...
!
!       dpx is a double precision variable.
!         On entry dpx is the the derivative of the function at stp.
!         On exit dpx is unchanged.
!
...
!     quadratic steps is taken.
      ...
      if (fp .gt. fx) then
         theta = three*(fx - fp)/(stp - stx) + dx + dpx
         s = max(abs(theta),abs(dx),abs(dpx))
         gamma = s*sqrt((theta/s)**2 - (dx/s)*(dpx/s))
         if (stp .lt. stx) gamma = -gamma
         p = (gamma - dx) + theta
         q = ((gamma - dx) + gamma) + dpx
         r = p/q
         stpc = stx + r*(stp - stx)
...
!     secant step is taken.
         ...
         p = (gamma - dpx) + theta
         q = ((gamma - dpx) + gamma) + dx
         r = p/q
         stpc = stp + r*(stx - stp)
         stpq = stp + (dpx/(dpx - dx))*(stx - stp)
...
!     and the magnitude of the derivative decreases.
      ...
      else if (abs(dpx) .lt. abs(dx)) then
...
!        to infinity in the direction of the step.
         ...
         gamma = s*sqrt(max(zero,(theta/s)**2-(dx/s)*(dpx/s)))
         if (stp .gt. stx) gamma = -gamma
         p = (gamma - dpx) + theta
         q = (gamma + (dx - dpx)) + gamma
         r = p/q
...
!     otherwise the cubic step is taken.
         ...
         if (brackt) then
            theta = three*(fp - fy)/(sty - stp) + dy + dpx
            s = max(abs(theta),abs(dy),abs(dpx))
            gamma = s*sqrt((theta/s)**2 - (dy/s)*(dpx/s))
            if (stp .gt. sty) gamma = -gamma
            p = (gamma - dpx) + theta
            q = ((gamma - dpx) + gamma) + dy
            r = p/q
            stpc = stp + r*(sty - stp)
            stpf = stpc
         else if (stp .gt. stx) then
