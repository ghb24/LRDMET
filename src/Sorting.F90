...
module sort_mod
    ...
    subroutine sort_d_a_c_a_r (arr, arr2, arr3, length, length2, length3)
        implicit none
        ...
        integer, parameter :: nInsertionSort = 7
        integer, parameter :: nskip_ = 1
        integer, intent(in) :: length, length2, length3

        real(dp), intent(inout) :: arr(length)
        complex(dp), intent(inout) :: arr2(length2,length)
        real(dp), intent(inout) :: arr3(length3,length)
    ...
    subroutine Orthonorm_zgeev_vecs(N,W,LVec,RVec)
        ...
        integer :: i,StartingInd,R,j,k,l
        complex(dp) :: norm,overlap
    ...
    !Perform two-sided modified gram-schmidt to bi-orthogonalize the degenerate vectors
            do j = StartingInd,i
                do k = StartingInd,j-1    !Bi-orthogonalize against the previous vectors
                    overlap = zzero
                    do l = 1,N
                        overlap = overlap + dconjg(LVec(l,k))*RVec(l,j)
                    enddo
                    !overlap = zdotc(N,LVec(:,k),1,RVec(:,j),1)
                    RVec(:,j) = RVec(:,j) - RVec(:,k)*overlap
                    overlap = zzero
                    do l = 1,N
                        overlap = overlap + dconjg(RVec(l,k))*LVec(l,j)
                    enddo
    ...
    !            
            do R = StartingInd,i
                ...
                norm = zzero
                do l = 1,N
                    norm = norm + dconjg(LVec(l,R))*RVec(l,R)
                enddo
                !norm = zdotc(N,LVec(:,R),1,RVec(:,R),1)
                norm = sqrt(norm)
                !write(6,*) "Norm: ",R,norm
